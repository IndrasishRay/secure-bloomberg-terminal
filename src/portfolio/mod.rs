...
impl PortfolioManager {
    ...
    pub fn buy(&self, symbol: &str, quantity: f64, price: f64) -> Result<Trade> {
        ...
        if total > portfolio.cash_balance {
            anyhow::bail!(
                "insufficient funds: need {total:.2}, have {:.2}",
        ...
        self.db
            .update_cash(self.portfolio_id, portfolio.cash_balance)?;
        ...
        match position {
            ...
            None => {
                self.db
                    .create_position(self.portfolio_id, symbol, quantity, price)?;
            }
        }

        let trade = self
            .db
            .create_trade(self.portfolio_id, symbol, "buy", quantity, price)?;
        audit::log_event(
            &self.db,
            "BUY",
            "user",
            &format!("{} {} @ ${:.2}", quantity, symbol, price),
        )
        .ok();
        Ok(trade)
    }
    ...
    pub fn sell(&self, symbol: &str, quantity: f64, price: f64) -> Result<Trade> {
        ...
        let position = self
            .db
            .get_position(self.portfolio_id, symbol)
            .ok_or_else(|| anyhow::anyhow!("position not found for {symbol}"))?;

        if quantity > position.quantity {
            anyhow::bail!(
                "insufficient shares: have {:.2}, want {quantity:.2}",
                position.quantity
            );
        }
        ...
        if remaining < 0.0001 {
            ...
            self.db
                .update_position(position.id, remaining, position.avg_cost, val)?;
        }

        let trade = self
            .db
            .create_trade(self.portfolio_id, symbol, "sell", quantity, price)?;
        audit::log_event(
            &self.db,
            "SELL",
            "user",
